        ...
        with open('Z:/temp.csv', 'r', encoding='utf-8-sig')as f:
            ...
            for index, row in enumerate(csvr):
                ...
                if nums % 10000 == 0:
                    ...
                    print(f'nums= {nums} 成功添加店铺{shops}  食品{foods}  分类{categorys}')
                nums += 1
                ...
                if index == 0:
                    ...
                    try:
                        if "categoryId" in indexlist:
                            strs = f"""{row[0]},{row[11]},'{row[12]}' , {row[13]} , {row[14]} , {row[15]} , {row[
                                16]} , {row[17]} , {row[18]} , {row[19]} , {row[8]} """
                            c.execute(
                            ...
                            c.execute(
                                f"""INSERT INTO food (ID,foodId,foodName,price,rating,monthSales,ratingCount,statisfyCount,statisfyRate,minPurchase,categoryId) 
                             VALUES ({row[0]},{row[8]},'{row[9]}' , {row[10]}, {row[11]},{row[12]},{row[13]},{row[14]},{
                                row[15]} ,{row[16]} ,NULL  )""")
                    except:
                        pass
                    else:
                        ...
                        try:
                            ...
                            c.execute(
                                f"""INSERT INTO shop (ID,name,address,description,latitude,longitude,phone,promotion_info) 
                                     VALUES ({row[0]},'{row[1]}','{row[2]}' , '{row[3]}', {row[4]},{row[5]},'{row[
                                    6]}' , '{row[
                                    7]}' )""");
                        except:
                            pass
